...
function insertPresidentsFromFile() {
    $.getJSON('users.json', function(users) {
        firebase.database().ref('bar/presidents').set(
            users
        );
    });
}
...
function updatePresident() {
    const adaNameRef = firebase.database().ref('bar/presidents/0');
    adaNameRef.update({
        first: 'Ada'
    });
}
...
function pushPresident() {
    ...
    const presidentRef = firebase.database().ref('bar/presidents');
...
function getPresidents() {
    ...
    return firebase.database().ref('/bar/presidents').once('value').then(function(snapshot) {
